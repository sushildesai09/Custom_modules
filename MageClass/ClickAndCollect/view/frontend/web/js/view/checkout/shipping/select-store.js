...
define([
	...
	'MageClass_ClickAndCollect/js/model/shipping-save-processor/default'
], function (Component, ko, $, $t, modal, quote,sds) {
    'use strict';
       ...
       // isMapVisible: ko.observable(false),
        ...
        initialize: function () {
        	...
        	quote.shippingMethod.subscribe(function () {
            	if (quote.shippingMethod() && quote.shippingMethod().carrier_code == 'clickandcollect') {
           ...
           /* this.isMapVisible.subscribe(function (value) {
			...
			$('body').on('change', '#select-store', function() {
				$("#pickup-store").val($(this).val());
				sds.saveShippingInformation();
			})
			/*$("select[name=select-store]").change(function(){
				$("input[name=pickup_store]").val($(this).val());
			});*/
